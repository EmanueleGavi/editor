...
import {
  ...
  usePrevious,
  wn
} from "./chunk-INPACG5Q.js";
import "./chunk-V4OQ3NZ2.js";

// node_modules/bpmn-js-properties-panel/node_modules/min-dash/dist/index.js
...
// node_modules/bpmn-js-properties-panel/node_modules/domify/index.js
var wrapMap = {
...
// node_modules/bpmn-js-properties-panel/node_modules/min-dom/dist/index.js
...
var index2 = getDefaultExportFromCjs(componentEventExports);
...
// node_modules/bpmn-js-properties-panel/node_modules/ids/dist/index.js
...
// node_modules/@bpmn-io/extract-process-variables/node_modules/min-dash/dist/index.js
...
function toMatcher2(matcher) {
  return isFunction2(matcher) ? matcher : (e) => {
    return e === matcher;
  };
}
...
function getExtensionElements(element, type) {
  ...
  if (typeof extensionElements !== "undefined") {
    ...
    if (typeof extensionValues !== "undefined") {
      elements = filter2(extensionValues, function(value) {
...
function eachElement(elements, fn, depth) {
  depth = depth || 0;
  if (!isArray2(elements)) {
    elements = [elements];
  }
  forEach2(elements, function(s, i) {
    var filter3 = fn(s, i, depth);
    if (isArray2(filter3) && filter3.length) {
      eachElement(filter3, fn, depth + 1);
    }
  });
}
function add2(elements, e, unique) {
...
function selfAndFlowElements(elements, unique, maxDepth) {
  ...
  eachElement(elements, function(element, i, depth) {
    add2(result, element, unique);
    var flowElements = element.flowElements;
    {
      if (flowElements && add2(processedFlowElements, flowElements, unique)) {
        return flowElements;
      }
    }
  });
...
function getElement(elementId, rootElement) {
  ...
  return find2(allElements, function(element) {
    return element.id === elementId;
  });
}
function addVariableToList(variablesList, newVariable) {
  var foundIdx = findIndex2(variablesList, function(variable) {
...
function getScope$1(element, globalScope, variableName, includeSelf = false) {
  ...
  var scopedParent = find2(parents, function(parent) {
    return hasInputMapping(parent, variableName);
  });
...
function hasInputMapping(element, name) {
  return find2(getInputMappings(element), function(input) {
...
function extractInputMappings(options) {
  ...
  forEach2(elements, function(element) {
    ...
    forEach2(inMappings, function(mapping) {
      if (!mapping.target) {
        return;
      }
...
function extractOutputMappings(options) {
  ...
  forEach2(elements, function(element) {
    ...
    forEach2(outMappings, function(mapping) {
      if (!mapping.target) {
        return;
      }
      var newVariable = createProcessVariable(
...
function extractOutputCollections(options) {
  let elements = options.elements, containerElement = options.containerElement, processVariables = options.processVariables;
  if (!isArray2(elements)) {
    elements = [elements];
  }
  forEach2(elements, function(element) {
...
function getProcessVariables(containerElement, additionalExtractors = []) {
  ...
  const allPromises = [];
  forEach2([...extractors, ...additionalExtractors], function(extractor) {
...
async function getVariablesForScope(scope, rootElement, additionalExtractors = []) {
  ...
  var scopeVariables = filter2(allVariables, function(variable) {
    return variable.scope.id === scopeElement.id;
  });
  var parents = getParents(scopeElement);
  var parentsScopeVariables = filter2(allVariables, function(variable) {
    return find2(parents, function(parent) {
...
function getOutputMappings2(element) {
...
function getParents2(element, includeSelf = false) {
  var parents = includeSelf ? [element] : [];
  var current = element;
  while (current.$parent) {
    parents.push(current.$parent);
...
function eachElement2(elements, fn, depth) {
  depth = depth || 0;
  if (!isArray2(elements)) {
    elements = [elements];
  }
  forEach2(elements, function(s, i) {
    var filter3 = fn(s, i, depth);
    if (isArray2(filter3) && filter3.length) {
      eachElement2(filter3, fn, depth + 1);
    }
  });
}
function add3(elements, e, unique) {
  var canAdd = !unique || elements.indexOf(e) === -1;
  if (canAdd) {
    elements.push(e);
  }
...
function selfAndFlowElements2(elements, unique, maxDepth) {
  var result = [], processedFlowElements = [];
  eachElement2(elements, function(element, i, depth) {
...
function selfAndAllFlowElements2(elements, allowDuplicates) {
  return selfAndFlowElements2(elements, !allowDuplicates);
}
...
function createProcessVariable2(flowElement, name, defaultScope, targetSelf = false) {
  var scope = getScope2(flowElement, defaultScope, name, targetSelf);
  return {
    name,
    origin: [flowElement],
    scope
...
function getScope2(element, globalScope, variableName, includeSelf = false) {
  var parents = getParents2(element, includeSelf);
  var scopedParent = find2(parents, function(parent) {
    return hasInputParameter(parent, variableName);
  });
  return scopedParent ? scopedParent : globalScope;
}
function hasInputParameter(element, name) {
  return find2(getInputParameters(element), function(input) {
    return input.name === name;
  });
}
function combineArrays$12(a, b) {
  return a.concat(b);
}
function extractOutputParameters(options) {
  ...
  forEach2(elements, function(element) {
    ...
    forEach2(outputParameters, function(parameter) {
...
function extractFormFields(options) {
  ...
  forEach2(elements, function(element) {
    ...
    forEach2(formFields, function(field) {
      var newVariable = createProcessVariable2(
        element,
        field.id,
        containerElement
...
function extractOutputMappings2(options) {
  var elements = options.elements, containerElement = options.containerElement, processVariables = options.processVariables;
  if (!isArray2(elements)) {
    elements = [elements];
  }
  forEach2(elements, function(element) {
    var outMappings = getOutputMappings2(element);
    forEach2(outMappings, function(mapping) {
      if (mapping.local) {
        return;
      }
...
function getEventDefinitions(element, type) {
  ...
  return filter2(eventDefinitions, function(definition) {
...
function extractEventDefinitionVariables(options) {
  ...
  forEach2(elements, function(element) {
    ...
    forEach2(errorEventDefinitions, function(definition) {
    ...
    forEach2(escalationEventDefinitions, function(definition) {
...
function getProcessVariables2(containerElement, additionalExtractors = []) {
  ...
  forEach2([...extractors2, ...additionalExtractors], function(extractor) {
    allPromises.push(
      extractor({
        elements,
        containerElement,
...
var DEFAULT_FEEL_LANGUAGE_CONTEXT = {
...
function BpmnPropertiesPanel(props) {
  ...
  const onLayoutChanged = T((newLayout) => {
  ...
  return u(BpmnPropertiesPanelContext.Provider, {
    ...
    children: u(FeelLanguageContext.Provider, {
...
function TimerEventDefinitionValue$2(props) {
  ...
  const commandStack = useService("commandStack"), translate = useService("translate"), debounce = useService("debounceInput");
...
var OPTIONAL_JOB_WORKER_ELEMENTS = ["bpmn:AdHocSubProcess", "bpmn:BusinessRuleTask", "bpmn:ScriptTask"];
function isZeebeServiceTask(element) {
  ...
  if (isAny(element, OPTIONAL_JOB_WORKER_ELEMENTS) && !getTaskDefinition$4(element)) {
    return false;
  }
  return true;
}
...
var BpmnFeelEntry = withVariableContext(withTooltipContainer(FeelEntry));
...
var BPMN_IMPLEMENTATION_OPTION = "bpmn";
var JOB_WORKER_IMPLEMENTATION_OPTION$3 = "jobWorker";
...
function VersionTag$3(props) {
  ...
  const bpmnFactory = useService("bpmnFactory"), commandStack = useService("commandStack"), debounce = useService("debounceInput"), translate = useService("translate");
...
function getCondition(element) {
  ...
  if (is(businessObject, "bpmn:Event")) {
    ...
    return conditionalEventDefinition == null ? void 0 : conditionalEventDefinition.get("condition");
  }
}
function getConditionBody(element) {
  const condition = getCondition(element);
  return condition == null ? void 0 : condition.get("body");
}
...
function ConditionExpression$1(props) {
  ...
  const commandStack = useService("commandStack"), bpmnFactory = useService("bpmnFactory"), translate = useService("translate"), debounce = useService("debounceInput");
  const getValue = () => {
    const condition = getCondition(element);
    return condition == null ? void 0 : condition.get("body");
  };
...
function VariableEvents$1(props) {
  ...
  const getValue = () => {
    ...
    const events = conditionalFilter == null ? void 0 : conditionalFilter.variableEvents;
...
function getConditionalFilter(element) {
  var _a;
  const conditionalEventDefinition = getConditionalEventDefinition$1(element);
  return (_a = getExtensionElementsList(conditionalEventDefinition, "zeebe:ConditionalFilter")) == null ? void 0 : _a[0];
}
...
function stringListToArray(string) {
  return (string == null ? void 0 : string.split(",").map((e) => e.trim()).filter((e) => e.length > 0)) ?? [];
}
function arrayToStringList(array) {
  return array.length > 0 ? array.join(",") : void 0;
}
...
function HeaderItem(props) {
  ...
  const headerId = `${idPrefix}-header-${index4}`;
...
function ExecutionListenersProps({
  ...
  const items = listeners.map((listener, index4) => {
...
function FormConfiguration(props) {
  ...
  const debounce = useService("debounceInput"), injector = useService("injector"), translate = useService("translate");
  const getValue = () => {
...
function InputProps$1({
  ...
  const items = inputParameters.map((parameter, index4) => {
    ...
    return {
      id,
      label: parameter.get("target") ? u("code", {
...
function removeFactory$c({
  ...
  return function(event2) {
    ...
    const businessObject = getBusinessObject(element), extensionElements = businessObject.get("extensionElements"), values = without(extensionElements.get("values"), ioMapping);
...
var EVENT_TYPE = ["creating", "assigning", "updating", "completing", "canceling"];
...
function TimerEventDefinitionValue$1(props) {
  ...
  const commandStack = useService("commandStack"), translate = useService("translate"), debounce = useService("debounceInput"), bpmnFactory = useService("bpmnFactory");
...
function indexEntriesById(entries) {
  return entries.reduce((index4, entry, idx) => {
    index4[entry.id] = idx;
    return index4;
  }, {});
}
...
function getParameters2(element, prop) {
...
function CreateParameterCmd(element, type, parent, bpmnFactory) {
  const isInput2 = type === "camunda:InputParameter";
  const newParameter = createElement(type, {
    name: nextId(isInput2 ? "Input_" : "Output_")
  }, parent, bpmnFactory);
  const propertyName = isInput2 ? "inputParameters" : "outputParameters";
  return {
    cmd: "element.updateModdleProperties",
    context: {
      element,
...
function MapProps(props) {
  ...
  const map2 = parameter.get("definition");
  const entries = map2.get("entries");
  function addEntry() {
    ...
    commandStack.execute("element.updateModdleProperties", {
      ...
      moddleElement: map2,
      properties: {
        entries: [...map2.get("entries"), entry]
      }
    });
  }
  function removeEntry(entry) {
    commandStack.execute("element.updateModdleProperties", {
      ...
      moddleElement: map2,
      properties: {
        entries: without(map2.get("entries"), entry)
      }
    });
  }
  return List({
...
function ConnectorInputProps(props) {
  ...
  const inputParameters = getInputParameters2(connector) || [];
  const items = inputParameters.map((parameter, index4) => {
...
function addFactory$5({
  ...
  return function(event2) {
    event2.stopPropagation();
    const businessObject = getBusinessObject(element);
    const error = createElement("camunda:ErrorEventDefinition", {}, void 0, bpmnFactory);
...
function getErrorLabel(errorEventDefinition) {
  ...
  const errorCode = error.get("errorCode"), name = error.get("name") || "<unnamed>";
  if (errorCode) {
...
function InMappingProps({
  ...
  const items = variableMappings.map((mapping, index4) => {
    ...
    return {
      id,
      label: mapping.get("target") ? u("code", {
...
function InputProps(props) {
  ...
  const inputParameters = getInputParameters2(element) || [];
  const bpmnFactory = injector.get("bpmnFactory"), commandStack = injector.get("commandStack");
  const items = inputParameters.map((parameter, index4) => {
...
function RetryTimeCycle(props) {
  ...
  const bpmnFactory = useService("bpmnFactory"), commandStack = useService("commandStack"), translate = useService("translate"), debounce = useService("debounceInput");
  const businessObject = getBusinessObject(element);
  const getValue = () => {
...
var TooltipProvider$1 = {
  "group-assignmentDefinition": (element) => {
    const translate = useService("translate");
    return u("div", {
      children: [translate("Define the assignment, schedule, and priority of the user task. Assignee, candidate groups, and candidate users can be combined. Use due date and follow-up date to express deadlines and urgency. "), u("a", {
  ...
  "group-condition": (element) => {
    const translate = useService("translate");
    return u("div", {
      children: [translate("Define a boolean condition expression that determines when this sequence flow is taken. "), u("a", {
  ...
  "group-businessRuleImplementation": (element) => {
    const translate = useService("translate");
    return u("div", {
      children: [translate("Evaluate a business rule, for example a DMN decision. To add a custom implementation, use a job worker. "), u("a", {
  ...
  "group-userTaskImplementation": (element) => {
    const translate = useService("translate");
    return u("div", {
      children: [translate("Implement a user task managed by Camunda. To add a custom implementation, use a job worker. "), u("a", {
  ...
  "group-form": (element) => {
    ...
    if (is(element, "bpmn:StartEvent")) {
      return u("div", {
        children: [translate("Link a form created with the Camunda Forms editor. Submitting this form will start a new process instance. "), u("a", {
  ...
  "group-calledDecision": (element) => {
    const translate = useService("translate");
    return u("div", {
      children: [translate("Define the decision to evaluate and how to map back the evaluation result. "), u("a", {
  ...
  "group-error": (element) => {
    ...
    if (is(element, "bpmn:EndEvent")) {
      return u("div", {
        children: [translate("Define the error to throw. Specify an error code (e.g. "), u("code", {
          children: "order-not-found"
        }), translate(") to identify the error. "), u("a", {
    ...
    return u("div", {
      children: [translate("Define the error to catch. Specify an error code (e.g. "), u("code", {
        children: "order-not-found"
      }), translate(") to match a specific error, or leave empty to catch any error. "), u("a", {
        href: "https://docs.camunda.io/docs/components/modeler/bpmn/error-events/#defining-the-error",
        target: "_blank",
        rel: "noopener noreferrer",
        title: translate("Error event documentation"),
  ...
  "priorityDefinitionPriority": (element) => {
    ...
    return u("div", {
      children: u("p", {
        children: [translate("Specifies the priority of a user task. Must be an integer between 0 and 100. If no value is provided, the default value is 50. "), u("a", {
  ...
  "group-activeElements": (element) => {
    const translate = useService("translate");
    return u("div", {
      children: [translate("Define a collection of elements which will be activated when the ad-hoc subprocess is reached. "), u("a", {
  ...
  "group-event-condition": () => {
    const translate = useService("translate");
    return u("div", {
      children: [translate("Specify the condition that triggers this event. "), u("a", {
  ...
  "condition": () => {
    const translate = useService("translate");
    return u("div", {
      children: [translate("A FEEL expression that determines whether the event should trigger. "), u("a", {
  ...
  "variableEvents": () => {
    const translate = useService("translate");
    return u("div", {
      children: [translate("Variable events that trigger the condition evaluation. If none selected, all variable events will trigger the condition evaluation. "), u("a", {
  ...
  "group-headers": (element) => {
    const translate = useService("translate");
    return u("div", {
      children: [translate("Define static metadata passed to job workers alongside each job. Headers are not evaluated by the engine — use them for fixed worker configuration (e.g. "), u("code", {
  ...
  "group-Zeebe__TaskListeners": (element) => {
    const translate = useService("translate");
    return u("div", {
      children: [translate("React to user task lifecycle events by executing a job worker. "), u("a", {
  ...
  "group-Zeebe__ExecutionListeners": (element) => {
    const translate = useService("translate");
    return u("div", {
      children: [translate("Add custom logic that reacts to various events in the workflow execution life-cycle. "), u("a", {
  ...
  "group-script": (element) => {
    const translate = useService("translate");
    return u("div", {
      children: [translate("Define the FEEL expression to evaluate and the variable to store the result in. "), u("a", {
        href: "https://docs.camunda.io/docs/components/modeler/bpmn/script-tasks/#defining-a-script-task",
        target: "_blank",
        rel: "noopener noreferrer",
        title: translate("Script task documentation"),
  ...
  "group-outputPropagation": (element) => {
    const translate = useService("translate");
    return u("div", {
      children: [translate("Control which variables from the called process are merged back into the parent scope. By default, all child variables are propagated; use output mappings to selectively return specific variables. "), u("a", {
  ...
  "group-adHocSubProcessImplementation": (element) => {
    const translate = useService("translate");
    return u("div", {
      children: [translate("Implement the ad-hoc subprocess as a job worker instead of activating inner elements. The job type determines which worker handles the subprocess execution. "), u("a", {
  ...
  "group-outputCollection": (element) => {
    const translate = useService("translate");
    return u("div", {
      children: [translate("Define how to collect the outputs of the ad-hoc subprocess. The output collection variable stores the list of results; the output element expression defines what each element contributes. "), u("a", {
        href: "https://docs.camunda.io/docs/components/modeler/bpmn/ad-hoc/",
        target: "_blank",
        rel: "noopener noreferrer",
        title: translate("Ad-hoc subprocess documentation"),
...
export {
  ...
  index3 as CamundaPlatformPropertiesProviderModule,
  TooltipProvider as CamundaPlatformTooltipProvider,
  index$12 as ZeebePropertiesProviderModule,
  TooltipProvider$1 as ZeebeTooltipProvider,
  useService
};
//# sourceMappingURL=bpmn-js-properties-panel.js.map
